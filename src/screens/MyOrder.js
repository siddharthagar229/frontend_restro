...
export default function MyOrder() {
    ...
    const fetchMyOrder = async () => {
        ...
        await fetch("https://backend-restro-bd.onrender.com/api/auth/myOrderData", {
            // credentials: 'include',
                                                ...
                                                <div  >
                                                    {arrayData.Order_date ? <div className='m-auto mt-5'>
                                                        ...
                                                        <div className='col-12 col-md-6 col-lg-3' >
                                                            <div className="card mt-3" style={{ width: "16rem", maxHeight: "360px", marginBottom:"30px"}}>
                                                                <img src={arrayData.img} className="card-img-top" alt="..." style={{ height: "120px", objectFit: "fill" }} />
                                                                <div className="card-body">
                                                                    <h5 className="card-title fst-san-serif">{arrayData.name}</h5>
                                                                    <div className='container w-100 p-0' style={{ height: "38px" }}>
                                                                        <span className='m-1 fst-san-serif'>{arrayData.qty}</span>
                                                                        <span className='m-1 fst-san-serif'>{arrayData.size}</span>
                                                                        <span className='m-1 fst-san-serif'>{data}</span>
                                                                        <div className=' d-inline ms-2 h-100 w-20 fs-5 fst-san-serif'>
                                                                            ₹{arrayData.price}/-
                                                                        </div>
                                                                    </div>
                                                                </div>
